    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        for legal_position in self.legalPositions:
            newPostDist = self.getPositionDistribution(self.setGhostPosition(gameState,legal_position))
            for newPos, prob in newPostDist.items():
                newBeliefs[newPos] += prob * self.beliefs[legal_position]
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        """
        "*** YOUR CODE HERE ***"
      #  ghostnum = [x for x in range(self.numGhosts)]
       # ghostPos = [p for p in self.legalPositions]
        x = 0
        self.particles = []
    ...
    def observeState(self, gameState):
        ...
        self.UpdateCapturedGhost(noisyDistances)
        ...
        for p in self.particles:
            ...
            for i in range(self.numGhosts):
                if noisyDistances[i] != None:
                    ...
                    weights[p] *= emissionModels[i][trueDistance]      
        ...
        if weights.totalCount() == 0:
            ...
            for x in range(self.numParticles):
                temp.append(util.sample(beliefs))
            self.particles = temp
            
    def UpdateCapturedGhost(self, noisyDistances):
        for i in range(self.numGhosts):
            if noisyDistances[i] == None:
                temp = []
                for p in self.particles:
