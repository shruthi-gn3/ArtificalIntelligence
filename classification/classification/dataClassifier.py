...
def enhancedFeatureExtractorDigit(datum):
    ...
    for w in range(width):
        for h in range(height):           
            if datum.getPixel(w,h) == 0 and (w,h) not in visited:
                fringe.push((w,h))
                visited.append((w,h))
                count += 1
                while not fringe.isEmpty():
                    curr_state = fringe.pop()
                    for p,q in neighbours(curr_state):
                        if (p,q) not in visited and (p>=0 and q>=0 and p < width and q < height):
                            if datum.getPixel(p,q)==0:
                                fringe.push((p,q))
                                visited.append((p,q))
                
                    
    if count <=3:
        features_new[count-1] =1
    

    "*** YOUR CODE HERE ***"
 

    return features + util.Counter(features_new)
...
def enhancedPacmanFeatures(state, action):
    ...
    features['numFoodLeft'] = nextState.getNumFood()

    Ghost_distance = []
    g_distance = 0
    
    ghostLocations = nextState.getGhostPositions()
    for l in ghostLocations:
        Ghost_distance.append(util.manhattanDistance(l, nextState.getPacmanPosition()))

    if min(Ghost_distance) > 0:
        g_distance = min(Ghost_distance)


    features['nearestGhostDist'] = g_distance

    food_distance = []
    f_distance = 0
    foodLocations = nextState.getFood().asList()
    for l in foodLocations:
        food_distance.append(util.manhattanDistance(l, nextState.getPacmanPosition()))
    if food_distance:
        f_distance = 1/min(food_distance)
        
    features['nearestFoodDist'] = f_distance   
