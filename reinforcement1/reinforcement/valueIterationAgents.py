...
class ValueIterationAgent(ValueEstimationAgent):
                ...
                if self.mdp.isTerminal(s):
                    ...
                    for a in actions:
                        ...
                        for t in transitions:
                            value +=  t[1] * (self.mdp.getReward(s,a,t[0]) + self.discount * tempVal[t[0]])
                        if value > maxValue:
                            maxValue = value
    ...
    def computeActionFromValues(self, state):
        ...
        if self.mdp.isTerminal(state):
            ...
            transitions = []
            maxValue = (-99999999,None)
            for a in actions:
                transitions = self.mdp.getTransitionStatesAndProbs(state, a)
                value = 0
                for t in transitions:
                    value += t[1] * (self.mdp.getReward(state, a, t[0]) + self.discount * self.values[t[0]])
                if value > maxValue[0]:
                    maxValue = (value,a)
            return maxValue[1]
